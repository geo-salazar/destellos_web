...
// 2. Consultamos los productos de la categoría 'Ofertas'
try {
...
<!-- Galería de ofertas dinámicas desde la BD -->
    ...
    <div class="cards-container">
        ...
        <?php else: ?>
            <?php foreach ($productos as $producto): ?>
                <article class="card">
                    <img src="<?= htmlspecialchars($producto['imagen_url'] ?? 'img/oferta.svg') ?>" 
                         alt="<?= htmlspecialchars($producto['nombre']) ?>">
                    <div class="card-content">
                        <h3><?= htmlspecialchars($producto['nombre']) ?></h3>
                        <p class="description"><?= htmlspecialchars($producto['description'] ?? 'Precio especial por temporada.') ?></p>
                        <p class="price">₡<?= number_format($producto['precio'], 0, ',', '.') ?></p>
                        <button onclick="addToCart('<?= htmlspecialchars($producto['nombre']) ?>', <?= $producto['precio'] ?>, '<?= htmlspecialchars($producto['imagen_url'] ?? 'img/oferta.svg') ?>')">Agregar al carrito</button>
                    </div>
                </article>
            <?php endforeach; ?>
        <?php endif; ?>
